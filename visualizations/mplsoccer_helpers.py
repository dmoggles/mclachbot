...
def make_grid(pitch: Pitch, figheight=9, nrows=1, ncols=1, left=None, grid_width=0.95,
        ...
        if (nrows > 1) and (ncols > 1):
            figwidth = figheight * grid_height / grid_width * (((1 - space) * pitch.ax_aspect *
                                                                ncols / nrows) +
        ...
        left_pad = (np.abs(pitch.visible_pitch - pitch.extent)[0] /
                    np.abs(pitch.extent[1] - pitch.extent[0])) * individual_pitch_width
        right_pad = (np.abs(pitch.visible_pitch - pitch.extent)[1] /
                     np.abs(pitch.extent[1] - pitch.extent[0])) * individual_pitch_width
